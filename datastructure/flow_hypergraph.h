...
namespace whfc {
	...
	public:
				...
				total_node_weight(0)
		{
			...
			for (Hyperedge e : hyperedgeIDs()) {
				...
				for (auto pin_it = beginIndexPins(e); pin_it != endIndexPins(e); pin_it++) {
					InHeIndex ind_he = nodes[pins[pin_it].pin].first_out++;							//destroy first_out temporarily and reset later
					pins[pin_it].he_inc_iter = ind_he;					//set iterator for incident hyperedge -> its position in incident_hyperedges of the node
		...
		PinIndexRange pinsNotReceivingFlowIndices(const Hyperedge e) const {
			if (forwardView()) {
				return PinIndexRange(beginIndexPins(e), pins_receiving_flow[e].begin());
			} else {
				return PinIndexRange(pins_receiving_flow[e].end(), endIndexPins(e));
			}
		}
		
		PinRange pinsNotReceivingFlowFrom(const Hyperedge e) {
			return pinsInRange(pinsNotReceivingFlowIndices(e));
		}
